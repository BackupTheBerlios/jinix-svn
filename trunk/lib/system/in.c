...
{
	uint8_t ret; // variable to return
	asm volatile("inb %1, %0" : "=a" (ret) : "dN" (port)); // send the opcode
	return ret; // return the value
}

uint16_t inw(uint16_t port) // inword
{
	uint16_t ret; // variable to return
	asm volatile("inw %1, %0" : "=a" (ret) : "dN" (port)); // send the opcode
	return ret; // return the value
}

uint32_t inl(uint16_t port) // inlong
{
	uint32_t ret; // variable to return
	asm volatile("inl %1, %0" : "=a" (ret) : "dN" (port)); // send the opcode
	return ret; // return the value
}
